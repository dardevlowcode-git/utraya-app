...
describe('fetchMainTranscript', () => {
  ...
  it('sceglie la manuale e compone testo e segmenti dal json3', async () => {
    ...
    expect(result?.is_asr).toBe(false)
  ...
  it('ritorna null quando il player non espone captionTracks', async () => {
    const fetchMock = vi.fn().mockResolvedValueOnce(jsonResponse({ playabilityStatus: { status: 'OK' } }))

    await expect(fetchMainTranscript('yt-2', fetchMock as unknown as typeof fetch)).resolves.toBeNull()
